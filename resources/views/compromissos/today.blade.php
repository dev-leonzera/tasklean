    ...
    <div class="col-12">
        <div class="d-flex flex-column flex-lg-row justify-content-between align-items-start align-items-lg-center gap-3">
            <div>
                ...
                <p class="text-muted mb-0">{{ now()->locale('pt_BR')->translatedFormat('l, d \d\e F \d\e Y') }}</p>
            </div>
            <div class="d-flex flex-wrap gap-2">
                <span class="badge bg-soft-info text-info px-3 py-2 rounded-pill fw-bold d-flex align-items-center" style="background: #e0f2fe">
                    {{ $compromissos->count() }} compromissos para hoje
                </span>
            </div>
        </div>
    </div>
...
@else
    <div class="chart-container text-center py-5">
        <div class="mb-4">
            <i class="bi bi-calendar-check text-success opacity-25" style="font-size: 5rem;"></i>
        </div>
        <h3 class="fw-800 text-dark">Agenda livre para hoje!</h3>
        <p class="text-muted mb-4">Você não tem nenhum compromisso marcado para o dia de hoje.</p>
        <a href="{{ route('compromissos.create') }}" class="btn btn-primary px-5 py-3 rounded-pill fw-bold shadow-sm">
            <i class="bi bi-plus-lg me-2"></i> Agendar Algo
        </a>
    </div>
@endif
@endsection
